    ...
    public class WidgetManager : MonoBehaviour
    {
        ...
        public void PrepareWidgets()
        {
			...
			for (int i = 0; i < widgetLibrary.widgets.Count; ++i)
            {
                ...
                if (!PlayerPrefs.HasKey("Widget" + tempWidget.gameObject.name) && tempWidget.isOn == true)
                {
                    widgetSwitch.isOn = true;
                ...
                else if (!PlayerPrefs.HasKey("Widget" + tempWidget.gameObject.name) && tempWidget.isOn == false)
                {
                    widgetSwitch.isOn = false;
                    widgetSwitch.gameObject.SetActive(false);
                    widgetSwitch.gameObject.SetActive(true);
                ...
                else if (PlayerPrefs.GetString("Widget" + tempWidget.gameObject.name) == "enabled")
                {
                    widgetSwitch.isOn = true;
                    widgetSwitch.gameObject.SetActive(false);
                    widgetSwitch.gameObject.SetActive(true);
                ...
                else if (PlayerPrefs.GetString("Widget" + tempWidget.gameObject.name) == "disabled")
                {
                    widgetSwitch.isOn = false;
                    widgetSwitch.gameObject.SetActive(false);
                    widgetSwitch.gameObject.SetActive(true);
